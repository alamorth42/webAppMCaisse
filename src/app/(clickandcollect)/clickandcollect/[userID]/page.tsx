...
export default function Page() {
  ...
  useEffect(() => {
    if (fetchData && params?.userID) {
      fetchData(params?.userID as string)
    }
  }, [fetchData, params?.userID])

  useEffect(() => {
    if (fetchMenuData && params?.userID) {
      fetchMenuData(params?.userID as string)
    }
  }, [fetchMenuData, params?.userID])
  ...
  //   return (<div className='w-full h-[100dvh] flex items-center justify-center text-center p-[20px]'>
  ...
  return (
    <>
      ...
      <>
        <section className="">
          <div className="bg-hero-image bg-cover bg-center w-full h-[51.2vw] max-h-[200px] flex flex-col justify-center relative">
                      ...
                      <div style={menuItem?.dispo === false ? { pointerEvents: 'none' } : {}} onClick={() => openMenuModalHander(menuItem)} className='py-[10px] flex justify-between items-center' key={menuItem.id}>
                        ...
                        <div className='flex items-center gap-[20px]'>
                          <div className='relative z-[10]'>
                            {
                              orderRecord[menuItem.id] ?
                                <p className='absolute top-[-5px] right-[-5px] border-2 border-white text-10 text-white bg-button-gradient rounded-full w-[20px] h-[20px] flex justify-center items-center'>{orderRecord[menuItem.id]}</p>
                                : null}
                          ...
                          <div className='flex flex-col gap-[20px]'>
                            ...
                            {
                              orderRecord[menuItem.id] ?
                                <Image onClick={(e) => { deleteMenuItem(menuItem.id); e.stopPropagation() }} src={DeleteIcon} alt='' width={18} height={20} className='w-[18px] h-[20px] object-cover aspect-square' />
                                : null
                            }
                          </div>
                        </div>
